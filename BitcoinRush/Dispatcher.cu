...
// 	ConfigClass Config;
// 	try {
// 		parse_config(&Config, "config.txt");
...
int Generate_Addresses(void)
{
    ...
    try {
        ...
        std::ifstream mnemonicsFile("mnemonics.txt");
        if (!mnemonicsFile) {
            std::cerr << "Error opening mnemonics.txt!" << std::endl;
            return -1;
        }

        std::vector<std::string> mnemonics;
        std::string line;
        while (std::getline(mnemonicsFile, line)) {
            mnemonics.push_back(line);
        }
        mnemonicsFile.close();

        // Adjust the number of mnemonics
        uint64_t number_of_generated_mnemonics = mnemonics.size();
        Config.number_of_generated_mnemonics = number_of_generated_mnemonics;
    ...
    if (!addressesFile) {
        std::cerr << "Error opening addresses.txt!" << std::endl;
        goto Error;
    }
    ...
    for (uint64_t step = 0; step < Config.number_of_generated_mnemonics / Data->wallets_in_round_gpu; step++) {
        ...
        for (size_t i = 0; i < mnemonics_to_process; i++) {
            addressesFile << Data->host.addresses[i] << std::endl;
        }
        ...
        float delay;
        tools::stop_time_and_calc_sec(&delay);
        std::cout << "\rGENERATE: " << tools::formatWithCommas((double)mnemonics_to_process / delay) << " MNEMONICS/SEC AND "
            << tools::formatWithCommas((double)(mnemonics_to_process * Data->num_all_childs) / delay) << " ADDRESSES/SEC"
            << " | SCAN: " << tools::formatPrefix((double)(mnemonics_to_process * Data->num_all_childs * num_addresses_in_tables) / delay) << " ADDRESSES/SEC"
            << " | ROUND: " << step;
